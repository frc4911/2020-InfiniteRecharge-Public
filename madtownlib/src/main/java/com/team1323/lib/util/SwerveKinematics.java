...
public class SwerveKinematics {
	...
	private final double swerveDiagonal;

	public double[] wheelAngles = new double[4];
	public double[] wheelSpeeds = new double[4];

	public SwerveKinematics(double wheelbaseLength, double wheelbaseWidth, double swerveDiagonal) {
		if (wheelbaseLength < 0.0) {
			throw new IllegalArgumentException("Wheel base length must be positive");
		}

		if (wheelbaseWidth < 0.0) {
			throw new IllegalArgumentException("Wheel base width must be positive");
		}

		if (swerveDiagonal < 0.0) {
			throw new IllegalArgumentException("Swerve diagonal must be positive");
		}

		this.wheelbaseLength = wheelbaseLength;
		this.wheelbaseWidth = wheelbaseWidth;
		this.swerveDiagonal = swerveDiagonal;
	}

	public void calculate(double x, double y, double rotate){
		double A = x - rotate * (wheelbaseLength / swerveDiagonal);
	    double B = x + rotate * (wheelbaseLength / swerveDiagonal);
	    double C = y - rotate * (wheelbaseWidth / swerveDiagonal);
	    double D = y + rotate * (wheelbaseWidth / swerveDiagonal);
	    ...
	    max = Utils.normalize(max, frontLeftWheelSpeed);
	    max = Utils.normalize(max, rearLeftWheelSpeed);
	    max = Utils.normalize(max, rearRightWheelSpeed);
	    if(max > 1.0){
