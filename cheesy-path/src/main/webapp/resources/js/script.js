...
let animating = false;
let isCCW = false;  // Y positive in CCW?

const fieldWidth = 624 + 2 * (28 + 38); // inches
const fieldHeight = 323 + 2 * 18; // inches
const xOffset = 66;
const yOffset = fieldHeight / 2;
const width = 1038; //pixels
const height = 514; //pixels

const robotWidth = 36.0; // inches
const robotHeight = 36.0; // inches

const scaleFactor = 1.25; // shrinks field image and robot size
...
class Translation2d {
	...
	get drawY() {
	    if (isCCW) {
            ...
            return height - (yOffset - this.y) * (height / fieldHeight);
        }
	}
}
...
function fillRobot(position, heading, color) {
    ...
    let w = robotWidth * (width / fieldWidth) / scaleFactor;
    let h = robotHeight * (height / fieldHeight) / scaleFactor;
...
function init() {
    ...
    let widthString = (width / scaleFactor) + "px";
    let heightString = (height / scaleFactor) + "px";
...
//    let heightString = (height) + "px";
    ...
    document.getElementById('ccw').innerHTML = isCCW ? 'Y+ is CCW' : 'Y+ is CW';

	image = new Image();
	image.src = '/resources/img/2020-field.png';
	image.onload = function() {
		ctxBackground.drawImage(image, 0, 0, width, height);
		update();
	};
	imageFlipped = new Image();
	imageFlipped.src = '/resources/img/2020-fieldFlipped.png';
    rebind();
}
